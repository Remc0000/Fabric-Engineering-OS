...
Use this checklist to select and start a governed Microsoft Fabric delivery path. The guides remain canonical and never authorize production deployment. The optional [project bootstrap helper](Initialize-FabricProject.ps1) can plan or create initial consumer documentation without configuring GitHub, running Spec Kit, or replacing any human gate.

## Optional project bootstrap helper

Preview deterministic changes without writing:

```powershell
.\bootstrap\Initialize-FabricProject.ps1 `
    -ProjectName "My Fabric Project" `
    -Sponsor "Accountable sponsor" `
    -SpecificationOwner "Human specification owner" `
    -RepositoryOwner "Repository team" `
    -SpecKitRelease "vX.Y.Z"
```

After reviewing the plan, pass `-Mode Apply` to create three files under `consumer/<project-slug>/`. Identical reruns are unchanged; differing files are conflicts and are never overwritten. Pass `-Mode Rollback` only to remove unmodified files generated by the helper. Writes require a clean Git working tree unless a human explicitly accepts the risk with `-AllowDirtyWorkingTree`.
